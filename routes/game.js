...
router.createGame = function(_data, _actions){
  ...
  db.challenges.find({ "_id":new ObjectID(_data.challengeId) }).toArray(function (err, items) {
    ...
    if( _data.userId == items[0].challengerId ){

      // build array
      var gameUserIds = [items[0].challengerId];
      ...
      async.mapSeries(gameUserIds, function(id, callback) {
        ...
        dealInitDiscardCard(game, items[0].challengerId);
...
router.checkPlayersInGameRoom = function(_data, _actions){
  // get gameObj
  db.games.find({ "_id":new ObjectID(_data.gameId) }).toArray(function (err, items) {
    if(checkAllPlayersPresent(items[0].players)){
      ...
      db.games.update({ "_id":new ObjectID(_data.gameId) }, {'$set':{status:"inprogress"}}, function(err, res){
        ...
        // IDEALLY WANT TO DELETE CHALLENGE HERE - BUT HAVING MONGO ISSUES
        // make sure challenges collection exists
        db.collectionNames("challenges", function(err, names) {
          if(names.length > 0){
            // delete Challenge
            db.challenges.remove({_id:new ObjectID(items[0].challengeId)}, function(err, res){
...
router.setPlayerInGame = function(_data, _actions){
  //console.log("setplayeringame: " + _data.userId + " challID: " + _data.challengeId);
  db.games.update({ "challengeId":new ObjectID(_data.challengeId), "players.id":new ObjectID(_data.userId) }, {'$set':{"players.$.inGame":true}}, function(err, items){
...
router.getGameByChallengeId = function(_data, _actions){
...
router.getGameById = function(_data, _actions){
...
router.validateMove = function(_data, _actions){
...
// draw a card
router.drawCard = function(_data, _actions){
...
router.challengeUno = function(_data, _actions){
...
router.sayUno = function(_data, _actions){
  var gameId = _data.gameId,
    playerId = _data.userId;
  ...
  getGameById({gameId:gameId, playerId:playerId}, _actions, function(game){
    ...
    if(currPlayerHandLength == 2){
      ...
      if(checkValidMove(game.players[currPlayerIndex].hand[0], game.discardPile) ||
        ...
        db.games.update({ "_id":new ObjectID(gameId), "players.id":new ObjectID(playerId) }, {'$set':{"players.$.calledUno":true}}, function(err, items){
          (err === null) ? _actions.success(new coreData.SensitiveGame(game, playerId)) : _actions.error();
        });
      }else{
...
router.quitGame = function(_data, _actions){
  var gameId = _data.gameId,
    playerId = _data.userId;
  ...
  getGameById({gameId:gameId, playerId:playerId}, _actions, function(_game){
    ...
    db.games.update({ "_id":new ObjectID(_data.gameId), "players.id":new ObjectID(_data.userId) }, {'$set':{"players.$.inGame":false}}, function(err, items){
      if(err === null){
...
deleteGameById = function(_gameId,_actions){
  ...
  db.collectionNames("games", function(err, names) {
    if(names.length > 0){
      // delete Challenge
      db.challenges.remove({_id:new ObjectID(_gameId)}, function(err, res){
        if(res != ""){
          (err === null) ? _actions.success() : _actions.error();
        }
      });
...
function dealInitDiscardCard(_gameObj, _challengerId){
  ...
  //  "value":"skip"
  //};
  //    _gameObj.discardPile.push(redSkip);
  //  handleActionCard(_gameObj, redSkip, _challengerId);
