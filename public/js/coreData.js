...
define('coreData', ['jquery', 'knockout', 'socketio'], function ($, ko, io) {
  let self = coreData = {},
    ...
    // httpVal = "http",
    httpVal = "https", // change to "https" for heroku server
  ...
  const GamePlayerCurrUser = function (_playerData) {
  ...
  const GamePlayer = function (_playerData) {
    this.id = _playerData.id;
    this.username = _playerData.username;
    this.calledUno = _playerData.calledUno;
  ...
  self.Game.prototype.getSVGName = function () {
    const svgName = this.discardPile()[this.discardPile().length - 1].svgName,
  ...
  const getTimeSent = function (_date) {
  ...
  const popPlayersArr = function (_players) {
    const gamePlayers = [];
  ...
  const getCurrGamePlayer = function (_players) {
    let currPlayer = {};
    for (let i = 0, j = _players.length; i < j; i++) {
      // only add other players that arent current User to players array
      if (_players[i].username === currUser().username) {
  ...
  const getUsernamesChallenged = function (_data) {
    let usernamesChallenged = [];
    for (let i = 0, j = _data.usersChallenged.length; i < j; i++) {
      usernamesChallenged.push(_data.usersChallenged[i].username);
    }
    return usernamesChallenged.join(", ");
  };

  const createChallengeSentText = function (_data) {
    return `You challenged ${getUsernamesChallenged(_data)} (${_data.status})`;
  };

  const createChallengeRecText = function (_data) {
    return `Challenge from ${_data.challenger.username} (${getRecChallengeStatus(_data)})`
  };

  // UI Manipulation

  const getSenderColor = function (_sender) {
    return _sender === coreData.currUser().username ? "me-txt" : "sender-txt";
  };

  const getChallengeClass = function (_status) {
    let className = "";
  ...
  const getRecChallengeStatus = function (_challenge) {
    let status = null;
