...
define('game', ['jquery', 'knockout', 'coreData', 'util'], function ($, ko, coreData, util) {
  ...
  // TEMPORARY INIT
  const initGame = function (_gameObj) {
    ...
    getGameChatInterval = setInterval(function () {
      if (util.userInGame()) {
        // check new msgs
        util.getChatMsgs(gameObj().gameId(), coreData.gameChatMsgs);
      }
    }, 1000);
  ...
  const checkWinner = function (_gameObj) {
    // If there is a winner, end the game and display winner
    if (_gameObj.winner !== "") {
      clearInterval(getGameInterval); // clear get game interval
      clearInterval(getGameChatInterval); // clear get game chat interval
      util.userInGame(false);
  ...
  const updateView = function (_gameObj) {
  ...
  self.startGame = function (data, event) {
    ...
    coreData.gameSocket.emit('createGame', {
      ...
      if (createGameRes.msg == "success") {
        ...
        const checkPlayersInGameRoomInterval = setInterval(
          function () {
  ...
  const getGame = function (_gameId, _actions) {
  ...
  function checkPlayersInGameRoom(_gameId, _actions) {
    coreData.gameSocket.emit('checkPlayersInGameRoom', {
      gameId: _gameId,
      userId: coreData.currUser().id
    }, function (data) {
      if (data.msg == "success") {
  ...
  const draw = function () {
    ...
    if (gameObj().currPlayer().isMyTurn()) {
      coreData.gameSocket.emit('drawCard', {
  ...
  const validateMove = function (item, event) {
    // make sure it is my turn
    // ( obviously more detailed checking is done on the server as well )
    if (gameObj().currPlayer().isMyTurn()) {
      const svgName = event.target.getAttribute("data-id");
  ...
  const handleValidateMove = function (_reqData) {
  ...
  const displayWildcardChoices = function (_callback) {
    // show modal to select color
    const $wildCardModal = $('#wildCardModal');
